...
import { MESSAGE, Anim } from '../../Script/TypeManager';
...
export default class PlayerSync extends ZepetoScriptBehaviour {
    ...
    private* SendLocalPlayer(tick: number) {
        ...
        while (true) {
            ...
            if(state != CharacterState.Idle || pastIdleCount < pastIdleCountMax || this.onWonderStateChange()) {
                ...
                if (GameManager.instance.isGetPlayerDB) {
                    const playerDB = JSON.stringify(GameManager.instance.playerDB);
                    this._room?.Send(MESSAGE.PlayerDB, playerDB);
                }
            }
